...
- 标识操作系统管理的资源，这些资源一般包括网络连接、打开的文件、计时器和同步对象等。在日志服务器中 `Handles` 用来标识 `SED` 用来等待事件发生的套接字端口（socket endpoint）。日志服务器感兴趣的两种事件是 *`connection`* 事件和 *`read`* 事件，它们分别代表传入的客户端连接和日志记录数据。日志服务器为每个客户端维护一个连接，这些连接在服务器中用套接字句柄指代。【注：跟 Windows 平台的句柄和 Linux 平台的文件描述符是类似的概念】
...
- 当应用程序向 `ID` 登记具体的事件处理器（`CEH`）的时候，应用程序会指明该 `EH` 希望 `ID` 在关联的句柄上发生事件的时候通知它的事件类型。
...
1. 日志服务器（1）向 `ID` 注册 `Logging Acceptor` 来处理连接请求；
2. 日志服务器掉用`ID` 的 `handle_events` 方法（2）；
3. `ID` 调用同步事件多路分离操作 `select`（3）来等待连接请求或者日志数据到达；
4. 客户端连接（4）到日志服务器
5. `Logging Acceptor` 会收到 `ID` （5）关于这个新连接请求的通知；
6. `Logging Acceptor` 接收（6）这个新的连接；
7. `Logging Acceptor` 创建（7）一个 `Logging Handler` 来服务这个新的客户端；
8. `Logging Handler` 向 `ID` 注册（8）他的套接字句柄，并指示分发器当这个套接字“读就绪”的时候通知它。
...
1. 客户端发送（1）一个日志记录数据；
2. 当客户端的日志记录被操作系统加入套接字句柄的队列时，`ID` 会通知（2）套接字句柄关联的 `Logging Handler` ；
3. 记录被以非阻塞的方式接收（3）（步骤 2 和步骤 3 重复进行，直到日志记录被完全接收）；
4. `Logging Handler` 处理日志记录，并把结果写入（4）标准输出；
5. `Logging Handler` 将控制权交给（5） `ID` 的事件循环。

## 实现

这部分描述了如何使用 C++ 实现 Reactor 模式，下面描述的实现受到ACE通信软件框架中提供的可重用组件的影响。

【注：下面都是实现，就不翻译了，有时间放个自己写的 Java 实现】
